# coding=utf-8
"""
For a random variable XX taking integer values between 1 and nn, the
expected value of X is E(X)=∑nk=1k×Pr(X=k)E(X)=∑k=1nk×Pr(X=k). The
expected value offers us a way of taking the long-term average of
...
of 3.5 on the die (even though it's not possible to roll a 3.5). The
formula for expected value confirms that
E(X)=∑6k=1k×Pr(X=k)=3.5E(X)=∑k=16k×Pr(X=k)=3.5.
...
possible value a and maximum possible value b, then E(X)=a+b2E(X)=a+b2. You
may also wish to verify that for the dice example, if Y is the random variable
associated with the outcome of a second die roll, then E(X+Y)=7E(X+Y)=7.
